...
### 发帖质量门槛（evidence-editor-v4）

- **额度是上限，不是目标**。新安装默认 `enableQualityGate=true`、`minPublishScore=58`；58 是初始工程阈值，不是收益预测分。
- v4 首次启动先备份旧 settings 与 prompts 到 `data/backups/editorial-v4/`，再一次性启用质量优先、将发布门槛提升到至少 58，并取消强制三标签。保留调度开关、发布模式、模型渠道、发帖间隔与额度；升级后在 UI 再关闭门槛或调整分数，不会被后续重启覆盖。保存设置不再把开关强制改成 false。
- LIVE 运行在调用模型前检查：低于门槛、`low_signal`、缓存行情回退，记为 `skipped`；不调用 LLM、不上传图片、不消耗发布额度、不计为错误。
- 同一主角同一事件在 2 小时内再次出现，若价格变化不足 2% 且 1h 涨跌幅变化不足 1.5 个百分点，跳过。与已有发布记录的快照比较；旧记录缺少快照时保守跳过。该规则是粗粒度去重，不是完整事件识别。
- dry-run/preview 仍可生成弱信号正文，运行记录中的 `publishPolicy` 说明 LIVE 是否会放行。`marketEvent.publishable` 和 `publishDecision` 是候选元数据，不代表 LIVE 发布许可。
- 保存 `contentEvidence`：只用最新、连续、已收盘的 20 根 15m K 线。最近已收盘 1h 成交量，与此前 4h 的每小时平均成交量比较；优先使用 quote volume，缺失时全窗口统一使用 base volume。未收盘、过期、缺根、缓存或零基准不得伪造量比。
- 24h 成交额只表示日内规模，不证明短线放量、主力买入或资金净流入；已收盘区间高低也不是已验证支撑阻力。观察、解释、条件必须分开。
- 默认仅强制主角 Cashtag，参照按实际相关性使用，最多 3 种且来自当前 trio。可勾选“兼容旧版：强制三个币标签”。主角最多两次、参照各一次，保留可点击空格边界。
- 非方向帖不再强制价位结尾。禁止“赔率开关、交易价值明显上升、留在交易页”等模板文案；增加去掉币名和数字后的**全文**结构相似度检查（阈值 0.62），配合原开头检查。
- 对无量比依据的直接“成交正在放大”等说法增加窄范围校验；条件句或否定句不阻断。这是启发式辅助，不是完整事实审计。
- 新默认 Prompt 为 v4。启动时保留旧默认版本为备份；自定义命名的 Prompt 保持启用，但运行时追加 v4 标签规则与事实边界。
- A 级明确方向帖仍受每日上限和冷却限制；配图门槛与上传限制不变。
...
npm run check
npm test
npm run preview:editorial
```

`test:quality` 为离线测试，验证已收盘量比、数据缺失、去重、质量开关持久化、Prompt 升级，以及 LIVE 跳过不触发模型/发布/计数。发布器是 stub，不向 Binance 或模型服务发请求。`preview:editorial` 使用 mock 文案，不代表真实模型最终质量。

推送 `main` 继续通过现有 GitHub Actions 自动部署。部署先安装依赖并通过语法、回归和预览测试；在 VPS 备份 settings/prompts、构建镜像，再重启服务。健康检查重试等待就绪并校验 Git revision 与策略版本，避免启动竞态或旧版本误判。启动会迁移质量预设和内置 Prompt，但不会改变现有调度开关与 LIVE/preview 模式。回退需恢复旧代码及备份配置/Prompt。`/health` 只公开版本与非敏感编辑开关，不公开令牌或模型密钥。

#### 如何判断优化有没有效果

比较改动前后相近市场环境与发布时间的样本，按选题类型分别记录每篇发布后 24h 阅读、互动、可获取的交易转化和收益，同时记录每天发帖数、跳过数、模型成本。看单篇效率与日总收益，而不是只追求发满额度。没有实际收益与转化数据，不能断言低收入全部由文案造成，也不能保证换 Prompt 就会增收。
