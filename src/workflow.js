...
const { publishToBinanceSquare, SQUARE_UPLOAD_DAILY_LIMIT } = require('./publisher');
...
function countMediaUploadsToday(settings = {}) {
  const today = dayKey(new Date(), settings.timezone || 'Asia/Shanghai');
  return listRuns(800)
    .filter(r => r.status === 'published' && r.createdAt)
    .filter(r => dayKey(new Date(r.createdAt), settings.timezone || 'Asia/Shanghai') === today)
    .reduce((sum, r) => sum + Math.max(0, Number(r.media?.uploadCount ?? r.media?.images?.length ?? 0)), 0);
}
...
async function configuredMedia(settings = {}, pack = null, generated = null, { livePublish = false } = {}) {
  ...
  const dailyUploadCount = countMediaUploadsToday(settings);
  const remainingUploadQuota = Math.max(0, SQUARE_UPLOAD_DAILY_LIMIT - dailyUploadCount);
  if (remainingUploadQuota <= 0) {
    return {
      imagePaths: [], autoGenerated: false, dailyUploadCount,
      dailyUploadLimit: SQUARE_UPLOAD_DAILY_LIMIT,
      skippedReason: 'square_daily_upload_limit_reached'
    };
  }
  const paths = [];
  ...
  const maxCount = Math.max(1, Math.min(4, remainingUploadQuota, Number(settings.imagePathCount || 1)));
...
async function runOnceUnlocked(mode = 'dry-run', meta = {}) {
  ...
  try {
    ...
    const published = await publishToBinanceSquare(generated.text, { format: 'short', imagePaths });
    ...
    const row = appendRun({
      ...
      publishStatus: published.publishStatus, confirmation: published.confirmation,
      contentType: published.contentType, postFormat: published.format,
    ...
    if (settings.notifyTelegram) {
      const linkLine = published.url
        ? `🔗 帖子链接：${published.url}`
        : '⚠️ Binance 网关返回 504，但官方语义为已提交；未返回帖子链接，请勿自动重试。';
    ...
    const row = appendRun({
      ...
      postText: generated?.text, editorial: generated?.editorial || pack?.editorialDecision, error: err.message || String(err), meta
    });
...
async function runOnce(mode = 'dry-run', meta = {}) {
  if (activeRun) {
    const err = new Error(`run_in_progress:${activeRun.mode}:${activeRun.startedAt}`);
  ...
  const promise = runOnceUnlocked(mode, meta);
  activeRun = { ...descriptor, promise };
  try {
...
module.exports = {
  ...
  formatTime,
  hasBannedSymbol,
  countAutoImagePublishedToday,
  countMediaUploadsToday,
  decideAutoImageForLiveRun
};
