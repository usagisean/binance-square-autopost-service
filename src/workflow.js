...
async function configuredMedia(settings = {}, pack = null, generated = null, { livePublish = false } = {}) {
  if (settings.enableImagePosts !== true) return { imagePaths: [], autoGenerated: false, autoImageDailyCount: countAutoImagePublishedToday(settings), autoImageDailyLimit: Number(settings.autoImageMaxDaily || 5) };
  const paths = [];
  let autoGenerated = false;
  const autoImageDailyLimit = Math.max(0, Number(settings.autoImageMaxDaily ?? 5));
  const autoImageDailyCount = countAutoImagePublishedToday(settings);

  if (settings.autoGenerateImage === true) {
    if (livePublish && autoImageDailyCount >= autoImageDailyLimit) {
      return { imagePaths: [], autoGenerated: false, autoImageDailyCount, autoImageDailyLimit, skippedReason: 'auto_image_daily_limit_reached' };
    }
    ...
    return { imagePaths: paths.slice(0, 1), autoGenerated, autoImageDailyCount, autoImageDailyLimit };
  }
...
module.exports = { runOnce, formatTime, hasBannedSymbol, countAutoImagePublishedToday };
